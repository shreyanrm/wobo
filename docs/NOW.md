...
| The climb: the group re-chosen after every module, and a door a learner can reach it through (rule 1) | 54 | RETIRED 2026-09-16, never committed. Every call was metered on the learner's turns and nothing called it, so the device chooses instead (`groupFor`, `screens/course/climb.ts`; docs/LEARNING-MODEL.md "Who chooses"); recoverable from `39548ee9`. What it was: `climb.py` on the gateway is a pure selection over the chapter's pool: no model, no store write, no network, which is what makes re-choosing free. `curriculum.climb` sits on the registry at tier TINY with cache tier NONE, because a cached step hands one learner another learner's next module. 29 played tests against the real CBSE class 8 Science pool, and 25 more through the real door with a real token, the real meter and the real stored pool. The client half re-chooses through `groupFor`, with misses counted by the tally the product already keeps (`wobo/reteach.ts`), 7 played tests — **in the played test only**: measured 2026-09-16, nothing in the running app ever sets `stuckOn`, so on a real learner's phone this branch is reached with an empty set every time. See the open item below. A module that beat a learner twice never comes back, and what follows is never harder than what beat them |
...
| What | Where it is specified |
|---|---|
| **Wave 54's fix is live code that never fires.** Corrected 2026-09-16 BY MEASUREMENT; the claim this row used to make was wrong on the mechanism and is kept here so the correction is legible. WRONG: that the client carried "an older, weaker version" of the choice. It does not. `apps/web-pwa/src/curriculum/blueprint.ts` `groupFor` carries the FULL re-choice — `state.stuckOn` (the fifth list, "what just HAPPENED to them"), the architect's own `flow.stuck` routes via `routeOutOf`, and a depth floor so nothing harder is offered while a learner is still standing on a module that beat them. 35 played tests are green against the current tree: 7 in `curriculum/group-rechosen.test.ts`, 28 across `screens/course/tutor-never-leaves.test.ts` and `where-they-went-wrong.test.ts`, 596 `expect()` calls in total. TRUE, and narrower: **nothing in production ever sets `stuckOn`.** Outside `blueprint.ts`'s own type, its comments and its read at line 377, the name does not occur anywhere in the app — so the branch runs with an empty set on every call and the fix is unreachable at runtime. The one production site that knows a learner just missed, `noteConceptMiss`, is called from exactly one place: `screens/doubt/climb.ts:79`, the photo-question flow, never the course. And `Course.tsx:246` takes the pool only to feed the placement gate — it never calls `groupInPool` at all | wave 54's own open item, re-scoped by measurement. It is a WIRE, not a port: feed `stuckOn` from the reteach tally at the course's module boundary, then decide whether `masteryOf` (real, tested, called by no production code) is what ends a topic. **Still the next thing worth doing, and smaller than it looked** |
...
| What | Owner |
|---|---|
| `curriculum.climb` has no caller in the product. The door is built, the registry carries it and the SDK declares `climb()`, but no screen calls it, so the phone *could* re-choose locally through `groupFor` — except that nothing feeds it `stuckOn`, measured 2026-09-16 — and the gateway's climb answers nobody. Rule 1 is true twice over and reachable once | wave 54's own follow-on: either one screen call, or the two re-choosers are stated as one thing on purpose CLOSED 2026-09-16: the door is retired and the course calls `groupFor` at every module boundary, with `stuckOn` fed from the module tally (`screens/course/climb.ts`) |
...
| Turning the growth desk on | built, stopped and keyless; it needs a schedule, Search Console and poster credentials, and the switch in the console | credentials, then a switch |
| Counsel: the campaign id on a child's account | the growth law writes the campaign that brought an account onto it, counts only are ever shown; whether that is tracking under DPDP s.9(3) for under-18s belongs with the lawyer | a question for counsel |
