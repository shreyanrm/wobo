...
- **The confirmation says the tap is final.** A provider cannot restart a cancelled subscription (the resume route answers 409 `cannot_resume`), so the last line of the confirmation is the gateway's own: "Once it is cancelled it cannot be switched back on, so this is the one tap that counts." The cancelled card repeats it, because that is where the missing Resume has to be explained.
...
- **The panel says a plan renews when, and only when, the gateway says the row renews.** A provider-backed subscription is charged again on its own — `services/gateway/src/wobo_gateway/billing/plans.py` creates every one with a `total_count` of five years or sixty months — so the active card names the day and says the same amount is taken again, and the confirmation is what stops it. A row nobody is charging (an operator's grant, a plan bought in a store) simply runs to the end of the period paid for, and no line about it carries the word. The screen never works this out for itself: `plan_view` answers `renews`, and the copy reads it. This rule used to say the opposite, on `billing.py` rule 2 — that file was deleted with commit 692affc and the package that replaced it has the provider, the webhook and the charge.
