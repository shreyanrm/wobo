...
- The quick one: "{{topic_name}} takes about five minutes" (renders to "Fractions, part two, takes about five minutes", 45 characters). A fact about the work, not about the learner's absence. No number leading, no urgency, nothing to accept or claim. **Only when something measured the card (2026-09-17).** Nothing does yet: the cadence sends the chapter and the card and no time, and a default of five put the same unmeasured time in every subject. Until a card carries a measured time the subject is "{{topic_name}} is waiting" (docs/EMAILS-AND-ANIMATIONS.md §1), and to a parent "{{learner_first_name}}'s next card in {{topic_name}} is waiting".
...
- The quick one: "Your next card in {{topic_name}} is a short one, about five minutes." It carries the topic, so it could only have been written to this learner. It does not mention the 48 hours, because saying what we observed is narrating. With no measured time: "Your next card in {{topic_name}} is where you left it."
...
**Where each one is held (2026-09-17).** Twenty-six rules, twenty-five of them in CI and one by hand. All twenty-five CI assertions run over every rendered kind in `services/gateway/tests/test_mail_law.py`, except four that already had a test and are cited there instead of written twice: the postal and unverified-domain holds (`test_email.py`), the inbox law (`test_mail_nudge_jobs.py`, `test_mail_cadence.py`, `test_hospitality_jobs.py`) and idempotency (`test_email.py`). The rendered-message check stays a hand check. Five places where the tests knowingly read the law in a particular way, each named in that file: the welcome is held to the 200-word ceiling; the Sunday note's reply is a `mailto:` and the invite's "Not me" is a second link above the footer, both by name; the wish carries no button at all; a board's own acronym (CBSE) is not a run of capitals; and the Feedback-ID's first field keeps the kind's underscore, because the watch reads the kind back out of it. Token overlap for the preheader is measured as shared words over all words. The eight subjects and first lines are now the ones the templates send, and the launch mail exists as a template (`launch`) that nothing sends yet. Its way out is a signed link that names the waiting list row (`waiting_list.stop_link`): the reader has no account, so the stop route takes the address off the list rather than pointing at a sign-in, and `send_email` holds a launch mail without that link.

**The dark client, measured, and how to measure it again.** `apps/web-pwa/scripts/mail-contrast.ts` renders every kind as `test_mail_law.py` does, opens each in a headless browser at 390 and 1440 wide in both colour schemes, and reads every visible text node against the ground behind it, at 4.5:1 (3:1 for large type). It exits non-zero on anything under. Run on 2026-09-17: 23 kinds, 92 screens, 1,272 text nodes, lowest 5.78:1, none under. The two dark-scheme tests in `test_mail_law.py` pin what fixed it; the script is the measurement.
