...
def fb(right: str = "the two shares are the same size", wrong: str = "count the pieces again: eight of sixteen is still half the bar") -> specs.Feedback:
        ...
        steps=[
            specs.InteractionStep(
                ...
                primitive=specs.DropInteraction(
                    ...
                    tokens=[
                        specs.DropToken(id=f"t{i}", label=lbl, box=box(x=2 + 15 * i, y=45), belongs=zone, why="it covers the same share")
                        for i, (lbl, zone) in enumerate(
                            [("1/2", "z1"), ("2/4", "z1"), ("1/3", "z2"), ("2/6", "z2")]
                        )
                    ],
                    zones=[
                        specs.DropZone(id="z1", label="half the bar", box=box(x=5, y=5, w=40, h=25), accepts=["t0", "t1"], feedback=fb()),
                        specs.DropZone(id="z2", label="a third of the bar", box=box(x=55, y=5, w=40, h=25), accepts=["t2", "t3"], feedback=fb()),
                    ],
                    feedback=fb(),
                ),
            ...
            specs.InteractionStep(
                id="s2",
                beat="check",
                primitive=specs.RevealSpec(kind="reveal", trigger="onDone", what="both bars shade to the same length", feedback=fb()),
            ),
        ],
    )
        ...
        steps=[
            specs.InteractionStep(
                ...
                primitive=specs.SortInteraction(
                    ...
                    items=[
                        specs.SortItem(id=f"i{i}", label=lbl, box=box(x=5, y=2 + 14 * i, w=80, h=13.5), rank=i + 1, why="it answers the event before it")
                        for i, lbl in enumerate(["Rowlatt Act", "Jallianwala Bagh", "the call to boycott", "Chauri Chaura"])
                    ],
                    feedback=fb(wrong="Chauri Chaura is why the movement stopped, so it cannot come first"),
                ),
            ),
            specs.InteractionStep(
                id="s2",
                beat="fun",
                primitive=specs.TimerSpec(kind="timer", seconds=60, onExpire="reveal", feedback=fb()),
            ),
        ],
    )
        ...
        steps=[
            specs.InteractionStep(
                ...
                primitive=specs.SlideInteraction(
                    ...
                    **{"from": -8},
                    at=0,
                    feedback=fb(wrong="the curve left the axis, so there is nothing real to solve for"),
                ),
            ),
            specs.InteractionStep(
                id="s2",
                ...
                primitive=specs.CanvasMarkInteraction(
                    ...
                    tool="point",
                    targets=[specs.CanvasTarget(id="r1", x=32, y=40, why="one root"), specs.CanvasTarget(id="r2", x=68, y=40, why="the other root")],
                    need=2,
                    feedback=fb(),
                ),
            ),
        ...
        steps=[
            specs.InteractionStep(
                ...
                primitive=specs.BranchInteraction(
                    ...
                    options=[
                        specs.BranchOption(id="a", label="2/4", box=box(x=2, y=2, w=40, h=20), correct=True, teaches=""),
                        specs.BranchOption(id="b", label="1/3", box=box(x=55, y=2, w=40, h=20), correct=False, teaches="a third is a smaller share than a half"),
                    ],
                    feedback=fb(),
                ),
            ),
            specs.InteractionStep(
                id="s2",
                beat="check",
                primitive=specs.TapInteraction(kind="tap", prompt="tap the bar that matches", targets=["m1"], need=1),
            ),
        ],
    )
...
CORE = {
    ...
    "misconceptions": [{"wrong": "a bigger denominator means a bigger fraction", "counter": "1/3 is less than 1/2"}],
    "check": "given 3/4, name one fraction that covers the same share",
    "kind": "classify",
    "material": {
        "bins": [{"id": "z1", "label": "half the bar"}, {"id": "z2", "label": "a third of the bar"}],
        "tokens": [
        ...
        "options": [
            ...
            {"id": "o2", "label": "1/3", "correct": False, "teaches": "a third is a smaller share than a half"},
        ],
    },
}
...
def test_the_hit_floor_is_the_44_css_px_law_at_390() -> None:
    ...
    assert specs.MIN_HIT_UNITS == pytest.approx(44.0 / 3.3, abs=1e-9)
...
def test_a_branch_needs_a_right_a_wrong_and_a_wrong_that_teaches() -> None:
    ok = specs.BranchInteraction(
        ...
        prompt="which one",
        options=[
            specs.BranchOption(id="a", label="2/4", box=box(2, 2, 40, 20), correct=True, teaches=""),
            specs.BranchOption(id="b", label="1/3", box=box(55, 2, 40, 20), correct=False, teaches="a third is smaller"),
        ],
        feedback=fb(),
    )
    ...
    with pytest.raises(ValidationError):  # every option correct: nothing to discriminate
        specs.BranchInteraction(
            ...
            options=[
                specs.BranchOption(id="a", label="2/4", box=box(2, 2, 40, 20), correct=True, teaches=""),
                specs.BranchOption(id="b", label="3/6", box=box(55, 2, 40, 20), correct=True, teaches=""),
            ],
            feedback=fb(),
        )
    with pytest.raises(ValidationError):  # a wrong option that teaches nothing
        specs.BranchInteraction(
            ...
            options=[
                specs.BranchOption(id="a", label="2/4", box=box(2, 2, 40, 20), correct=True, teaches=""),
                specs.BranchOption(id="b", label="1/3", box=box(55, 2, 40, 20), correct=False, teaches="  "),
            ],
            feedback=fb(),
        )
...
def test_a_drop_zone_rule_must_name_tokens_that_exist() -> None:
    with pytest.raises(ValidationError):
        specs.DropInteraction(
            ...
            zones=[
                specs.DropZone(id="z1", label="half", box=box(5, 5, 40, 25), accepts=["t1", "ghost"], feedback=fb()),
                specs.DropZone(id="z2", label="third", box=box(55, 5, 40, 25), accepts=["t2"], feedback=fb()),
            ],
            feedback=fb(),
        )
...
def test_the_gate_refuses_a_repeat_of_the_chapters_last_three() -> None:
    ...
    verdict = validate.judge_interaction_design(
        repeat, core=CORE, recent=recent, judge_model="m", judge=lambda *a, **k: {"score": 95, "critical": False}
    )
    assert verdict.ok is False
...
def test_the_gate_refuses_targets_that_overlap_under_a_finger() -> None:
    ...
    verdict = validate.judge_interaction_design(
        d, core=CORE, recent=(), judge_model="m", judge=lambda *a, **k: {"score": 95, "critical": False}
    )
    assert verdict.ok is False
...
def test_the_gate_passes_a_design_that_moves_and_teaches() -> None:
    verdict = validate.judge_interaction_design(
        ...
        judge_model="m",
        judge=lambda *a, **k: {"score": 88, "critical": False, "weak": [], "notes": "the wrong drop teaches the share"},
    )
...
def test_the_gate_refuses_when_the_judge_scores_below_the_bar() -> None:
    verdict = validate.judge_interaction_design(
        ...
        recent=(),
        judge_model="m",
        judge=lambda *a, **k: {"score": 41, "critical": False, "weak": ["template"], "notes": "a fourteen-year-old would feel the template"},
    )
    assert verdict.ok is False
...
def test_below_the_bar_twice_takes_the_template_floor(monkeypatch) -> None:
    ...
    monkeypatch.setattr(engines, "_complete_design", fake)
    out = engines.design_interaction(CORE, recent=(), judge=lambda *a, **k: {"score": 99, "critical": False})
    ladder = generation_ladder()
...
def test_a_design_goes_stale_on_the_cadence(monkeypatch) -> None:
    ...
    assert validate.design_is_stale(record, now="2026-02-01T00:00:00+00:00", core_version="v1") is False
    assert validate.design_is_stale(record, now="2026-06-01T00:00:00+00:00", core_version="v1") is True


def test_a_core_change_makes_a_design_stale_at_once() -> None:
    record = {"designedAt": "2026-01-01T00:00:00+00:00", "refreshDays": 90, "coreVersion": "v1"}
    assert validate.design_is_stale(record, now="2026-01-02T00:00:00+00:00", core_version="v2") is True
...
def test_three_concepts_get_three_genuinely_different_mechanics() -> None:
    ...
    for d in designs:
        verdict = validate.judge_interaction_design(
            d, core=CORE, recent=recent, judge_model="m", judge=lambda *a, **k: {"score": 86, "critical": False}
        )
