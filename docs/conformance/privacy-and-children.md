...
| I36a | Storage bucket `doubt-photos` (`0021_doubts.sql`), object `<subject_id>/<doubt_id>.jpg` | a photograph of the child's own page of work: re-encoded by the gateway with its metadata emptied (EXIF, XMP, ICC and the JPEG comment marker; until 2026-09-05 only EXIF left, and a COM marker carrying an address survived), long edge at most 1600 px, and SCREENED before it was written (`doubt.py`, `safety.screen_image`, fails closed). What the screen does, exactly: a face is refused; a photo that is not a page of work is refused; a name or address ON the page is refused when the screen cannot find the work apart from it, and otherwise the photo is CROPPED to the work and the crop is screened again, and a crop that still carries details is refused (fixed 2026-09-05: a second 'details, but here is a crop' used to pass). So a page with a name in its header is kept, cropped, when the screen found the work apart from it; that is a model's judgment, not a guarantee | contract | until erase | `/v1/me/erase` (`memory.py`, `doubt.PostgrestDoubtStore.forget_all` lists every object under the learner's prefix a page at a time and reports the count the bucket CONFIRMED gone, not the number asked) and `DELETE /v1/doubt/{id}` |
| I36b | `learner.doubts` (`0021_doubts.sql`): `subject_id, words, school_subject, topic, question, lines (text + box per line), node_id, framework_id, status` | what Wobo read from the child's page, corrected by the child, their bounded words beside it, and where it was filed in their syllabus | contract | until erase | `/v1/me/erase` and `DELETE /v1/doubt/{id}`; listed to the learner on `GET /v1/doubt` |
...
**Count: 24 durable stores hold personal data — 14 `learner` tables (every one except
`content_cache`, `learner.doubts` since 0021), 7 `curriculum` tables, `auth.users`, the three
storage buckets of 0003 counted as one, and the `doubt-photos` bucket of 0021. Eight of the 24 are
reached by an erase (the two doubt stores among them, and the bucket is the first storage bucket
an erase reaches). The mail log, the in-process caches and the platform logs (I37–I42) are on top
of that.**
...
voice grants — and, since the doubt solver (`memory.py`, `doubt.py`), `learner.doubts` and every
object under the learner's prefix in the `doubt-photos` bucket, both counted in the answer. `eraseSubjectRows` (`packages/sdk/src/supabase.ts:46-53`) reaches: `learner_state`,
...
| L4 | Photographs to a provider | Art 44 | PARTIAL | A photographed syllabus page is base64'd into a data URL and sent to the image-capable generate tier (`curriculum/own.py:258-276`). The document text and its hash are kept; the page itself is not stored server-side. The UI warns "Never send pictures of people or personal information" (`OwnSyllabus.tsx:222-224`), which is the right instruction to a child. A doubt photo (`doubt.py`) goes to the screen's tier and the reader's chain (H1) and IS stored server-side, in `doubt-photos`, keyed to the account (I36a) | Gate photo intake on consent; ask each provider for zero-retention terms on the vision calls, or set the provider's no-store flag where one exists |
